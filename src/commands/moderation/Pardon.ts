...
export default class PardonCommand extends Command {
    ...
    async run(ctx: Context) {
        ...
        if (!PermissionUtils.above(ctx.message.member!, member)) return ctx.send('The user is above you in the heirarchy.');

        await this.bot.punishments.pardon(member!, amount);
        const warns = await this.bot.warnings.get(ctx.guild.id, member.id);
        if (!warns) return ctx.send(`Successfully pardoned ${member.username}#${member.discriminator}! They now have 0 warnings!`);
