...
export default class BanCommand extends Command {
    ...
    async run(ctx: Context) {
        ...
        if (!member || !(member instanceof Member)) member = { id: u, guild: ctx.guild };
        else {
        ...
        if (reason && typeof reason === 'boolean') return ctx.send('You need to specify a reason.');

        let time = (ctx.flags.get('time') || ctx.flags.get('t'));
        if (typeof time === 'boolean') return ctx.send('You need to specify time to be allotted.');
