...
export default class CommandManager {
     ...
     */
    constructor(@inject(TYPES.Bot) bot: Bot, @inject(TYPES.CommandService) commandservice: CommandService) {
        this.bot  = bot;
    ...
    async start() {
        ...
        for (let i = 0; i < groups.length; i++) {
            ...
            readdir(`${this.path}${sep}${category}`, (error, files) => {
                ...
                this.bot.logger.log('info', `Building ${files.length} command${files.length > 1? 's': ''}`);
                files.forEach((file) => {
                    try {
                        ...
                        this.bot.logger.log('error', `Couldn't initialize command ${file}. Error: ${err}`);
                    }
