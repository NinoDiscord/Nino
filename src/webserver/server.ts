...
export default class NinoWebServer {
  ...
  start() {
    this.app.get('/', (_, res) =>
      res
        .status(200)
        .send(this.responses[Math.floor(Math.random() * this.responses.length)])
    );
    this.app.get('/commands', (_, res) =>
      res.status(200).json({ success: true, data: this.getCommands() })
    );
    this.app.get('/commands/:cmd', (req, res) => {
      ...
      if (cmd === undefined)
        res
          .status(500)
          .json({ success: false, message: 'Command was not found.' });
      res.status(200).json({ success: true, data: cmd! });
    });

    this.app.listen(this.client.config.webserver, () =>
      this.client.logger.log(
        'info',
        `Web server listening on ${this.client.config.webserver}~`
      )
    );
  }
}
