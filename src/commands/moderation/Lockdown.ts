...
export default class LockdownCommand extends Command {
    ...
    async run(ctx: CommandContext) {
        ...
        if (!ctx.flags.get('roles')) return ctx.send('You must first specify the roles to change permissions to using the --roles flag!');
        ...
        if (roles.length === 0) return ctx.send('All of the roles you\'ve listed can not be modified by me! Please check that the roles are under this bot in the heirarchy.'); 
