...
export default class WarningsCommand extends Command {
    ...
    async run(ctx: Context) {
        ...
        if (!settings) return ctx.send(`${member.username}#${member.discriminator} has 0 warnings.`);
        else return ctx.send(`${member.username}#${member.discriminator} has ${settings!.amount} warnings.`);
    }
}