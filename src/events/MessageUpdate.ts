...
export default class MessageUpdatedEvent extends Event {
  ...
  async emit(m: Message<TextChannel>, old: OldMessage | null) {
    ...
    const embed = createEmptyEmbed()
      ...
      .addField('Old Content', old.content.length > 1993 ? `${old.content.slice(0, 1993)}...` : old.content || 'Nothing was sent?')
      .addField('New Content', m.content.length > 1993 ? `${m.content.slice(0, 1993)}...` : m.content || 'Nothing was sent?');
